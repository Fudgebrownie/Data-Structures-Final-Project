...
template <typename DataType>
void tree<int>::display( tree<int> * locptr ){
    if( locptr->left != nil )
        _display( locptr->left );
    if( locptr != nil ){
        cout << locptr-> << ' ';
   
        if( locptr->p != nil )
            cout << locptr->parent->data[] << ' ';
        else
            cout << "NULL ";
        if( locptr->left != nil )
            cout << locptr->left->data[] << ' ';
        else
            cout << "NULL ";
        if( locptr->right != nil )
            cout << locptr->right->data[] << ' ';
        else
            cout << "NULL ";
    }
    cout << endl;
    if( locptr->right != nil )
        display( locptr->right );
}
//
